...
async def post_artigo(artigo: ArtigoSchema, usuario_logado: UsuarioModel = Depends(get_current_user), db:AsyncSesion = Depends(get_session)):
    novo_artigo: ArtigoModel = ArtigoModel(
        titulo=artigo.titulo, descrição=artigo.descricao, url_fonte=artigo.url_fonte, usuario_id=usuario_logado.id)
...
async def get_artigo(artigo_id: int, db: AsyncSesion = Depends(get_session)):
    async with db as session:
...
async def put_artigo(artigo_id: int, artigo: ArtigoSchema, db: AsyncSesion = Depends(get_session), usuario_logado: UsuarioModel = Depends(get_current_user)):
    async with db as session:
        query = select(ArtigoModel).filter(ArtigoModel.id == artigo_id)
        result = await session.execute(query)
...
async def delete_artigo(artigo_id: int, db: AsyncSesion = Depends(get_session), usuario_logado: UsuarioModel = Depends(get_current_user)):
    async with db as session:
